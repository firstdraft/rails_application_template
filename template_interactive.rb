# Interactive Rails Application Template
# Usage:
#   rails new APP_NAME \
#     --database=postgresql \
#     --javascript=esbuild \
...
#     --skip-kamal \
#     --skip-rubocop \
#     -m /path/to/this/template_interactive.rb

base_url = "https://raw.githubusercontent.com/firstdraft/rails_application_template/main"

# === INTERACTIVE CONFIGURATION ===

say "\n=== Rails Application Template Configuration ===", :yellow
say "Let's customize your Rails app with the tools you need.\n", :yellow
...
# === ESSENTIAL GEMS (ALWAYS INCLUDED) ===
...
gem_group :development do
  ...
  # Documentation (always include annotaterb)
  gem "annotaterb"
...
gem_group :test do
  ...
  # Enhanced Capybara (always include)
...
after_bundle do
  ...
  if yes?("Use UUIDs for primary keys instead of integers? (y/n)")
  ...
  git add: "-A"
  git commit: "-m 'Configure RSpec with selected testing tools'"
  ...
  create_file "README.md", <<~MARKDOWN
    ...
    #{"bundle exec bundle-audit check" if security_options[:bundler_audit]}
    ```
    ...
    #{"- Generate ERD: `bundle exec erd`" if doc_options[:rails_erd]}
    - Annotate models: `bundle exec annotaterb models`
    #{"- View database: Visit `/rails_db` in development" if doc_options[:rails_db]}
  MARKDOWN
  ...
  say "\n🎉 Rails app successfully created with your selected options!", :green
